...
describe('DeleteWithTransferUseCase', () => {
  ...
  describe('pre-transaction validations', () => {
    it('should throw POCKET_NOT_FOUND when source pocket does not exist', async () => {
      ...
      await expect(useCase.execute('user-1', 'invalid-id', [{ targetPocketId: 'target-1', amount: 100 }], 'reason', new Date()))
        .rejects
        .toThrow('POCKET_NOT_FOUND:invalid-id');
    });
    ...
    it('should throw DISTRIBUTION_SUM_MISMATCH when sum does not match balance', async () => {
      ...
      await expect(
        useCase.execute('user-1', 'source-1', [{ targetPocketId: 'target-1', amount: 100 }], 'reason', new Date()),
    ...
    it('should throw SOURCE_IS_TARGET when distribution targets the source pocket', async () => {
      ...
      await expect(
        useCase.execute('user-1', 'source-1', [{ targetPocketId: 'source-1', amount: 500 }], 'reason', new Date()),
    ...
    it('should throw POCKET_NOT_FOUND when a target pocket does not exist', async () => {
      ...
      await expect(
        useCase.execute('user-1', 'source-1', [{ targetPocketId: 'missing-target', amount: 500 }], 'reason', new Date()),
  ...
  describe('atomic transaction', () => {
    ...
    it('should debit source, credit target, create deposit, and delete pocket', async () => {
      ...
      const mockDepositEntity = { id: 'dep-1', pocketId: 'target-1', amount: 500, date: new Date() };
      ...
      expect(mockEntityManager.create).toHaveBeenCalledWith('user-1', 
        DepositEntity,
    ...
    it('should throw INSUFFICIENT_FUNDS when balance changed after pre-validation', async () => {
      mockQueryBuilder.getOne
        .mockResolvedValueOnce(createMockEntity('source-1', 100)); // balance now only 100
