...
describe('GetPocketWithDepositsUseCase', () => {
  ...
  describe('execute', () => {
    it('should call pocketService.getPocketWithDeposits with correct id', async () => {
      const mockPocket = new Pocket('Savings', 'goal', 1000, 500, 'Test motivation');
      const mockDeposits = [
      ...
      expect(mockService.getPocketWithDeposits).toHaveBeenCalledWith('user-1', 'pocket-1');
      expect(result).toEqual({ pocket: mockPocket, deposits: mockDeposits, expenses: [] });
    });

    it('should propagate errors from getPocketWithDeposits', async () => {
      mockService.getPocketWithDeposits.mockRejectedValue(new Error('Pocket not found'));
