...
describe('UpdatePocketUseCase', () => {
  ...
  describe('execute', () => {
    it('should call pocketService.updatePocket with correct params', async () => {
      const mockPocket = new Pocket('Updated Savings', 'goal', 2000, 500, 'Test motivation');
      ...
      expect(mockService.updatePocket).toHaveBeenCalledWith('user-1', 'pocket-1', updates);
    ...
    it('should propagate errors from updatePocket', async () => {
      ...
      await expect(useCase.execute('user-1', 'invalid-id', { name: 'Test' })).rejects.toThrow(
        'Pocket not found',
      );
    });
  });
