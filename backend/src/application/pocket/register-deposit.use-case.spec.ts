...
describe('RegisterDepositUseCase', () => {
  ...
  it('should register a deposit successfully', async () => {
    const mockPocket = new Pocket('Savings', 'goal', 1000, 0, 'Test motivation');
    ...
    const result = await useCase.execute('user-1', 'pocket-1', 100, new Date('2024-01-15'));

    expect(mockRepo.findById).toHaveBeenCalledWith('user-1', 'pocket-1');
