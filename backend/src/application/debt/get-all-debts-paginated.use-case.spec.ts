...
describe("GetAllDebtsPaginatedUseCase", () => {
  ...
  describe("execute", () => {
    it("should call debtService.getAllDebtsPaginated with query", async () => {
      ...
      expect(mockService.getAllDebtsPaginated).toHaveBeenCalledWith(
        "user-1",
        mockQuery,
      );
    ...
    it("should propagate errors from getAllDebtsPaginated", async () => {
      const mockQuery: DebtQueryDto = { page: 1, limit: 10 };
      mockService.getAllDebtsPaginated.mockRejectedValue(
        new Error("Database error"),
      );

      await expect(useCase.execute("user-1", mockQuery)).rejects.toThrow(
        "Database error",
      );
    });
  });
