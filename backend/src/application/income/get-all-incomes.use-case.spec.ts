...
describe('GetAllIncomesUseCase', () => {
  ...
  describe('executePaginated', () => {
    ...
    it('should propagate errors from getAllIncomesPaginated', async () => {
      mockService.getAllIncomesPaginated.mockRejectedValue(new Error('Database error'));

      await expect(useCase.executePaginated('user-1', 0, 10)).rejects.toThrow('Database error');
    });
  });
