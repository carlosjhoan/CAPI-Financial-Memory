...
import { Income, IncomeFilters } from '../../features/incomes/types/income.types';
import {
  useIncomesPaginated,
  useMonthlySummary,
  useYearlySummary,
  useOverallSummary,
} from '../../features/incomes/hooks/useIncomes';
...
export interface UseIncomeSectionOptions {
  filters?: IncomeFilters;
  filterType?: SessionFilterType;
  onCreate?: () => void;
  card: CardRenderConfig<Income>;
  /** Opcional: sobrescribe layout config por defecto */
  layout?: Partial<LayoutConfig>;
  /** Handlers opcionales */
  onEdit?: (item: Income) => void;
  onDelete?: (item: Income) => void;
  onClick?: (item: Income) => void;
}

export interface IncomeSectionReturn {
  items: Income[];
  paginationMeta: EntityFinancialSectionProps<Income>['paginationMeta'];
  error: Error | null;
  viewMode: ViewMode;
  card: CardRenderConfig<Income>;
 ...
 * en el contexto de la sección de Ingresos.
 * Centraliza queries, paginación, selección de mes, etc.
 */
export function useIncomeSection({
  filters,
  filterType = 'all',
  onCreate,
  card,
  layout: layoutOverride,
}: UseIncomeSectionOptions): IncomeSectionReturn {
  ...
  const { data: timelineMonthData } = useIncomesPaginated(
  ...
  // Queries
  const { data: paginatedData, error } = useIncomesPaginated(filters, currentPage, LIMIT);
  ...
  // but GET /incomes?year=Y with meta.total gives us the count)
  const { data: yearlyCountData } = useIncomesPaginated(
  ...
  // Query for selected month incomes
  const { data: selectedMonthIncomes } = useIncomesPaginated(
  ...
  const summary: SummaryController = useMemo(() => ({
    ...
    overall: overallSummary ? {
      ...
      count: overallSummary.totalIncomes,
    } : null,
  ...
  const monthlyBreakdown: MonthlyBreakdownController = useMemo(() => ({
    ...
    selectedItems: selectedMonthIncomes?.data ?? [],
    selectedPagination: selectedMonthIncomes?.meta,
    onSelectedPageChange: setSelectedMonthPage,
  }), [selectedMonth, selectedMonthIncomes]);
  ...
  useAutoCorrectPage(selectedMonthIncomes?.meta, selectedMonthPage, setSelectedMonthPage);
  ...
  const dynamicTitle = useMemo(() => {
    if (viewMode === 'dateRange' && filters?.startDate && filters?.endDate) {
      ...
      return `Ingresos registrados (Desde ${startFormatted} hasta ${endFormatted})`;
    }
